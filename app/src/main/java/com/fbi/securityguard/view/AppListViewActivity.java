...
public class AppListViewActivity extends BaseActivity implements AppListViewInterface {
    ...
    private void iniEvent() {
        appInfoRecyclerAdapter.setOnItemClickListener(new AppInfoRecyclerAdapter.OnItemClickListener() {
            @Override
    ...
    private String getAppInfoDetail(AppInfo appInfo) {
        return getResources().getString(R.string.installTime) + ":" + DateUtils.getDateTime(appInfo.getAppInstallTime()) + "\n" + getResources().getString(R.string.versionCode) + ":" +
                appInfo.getVersionName() + "\n" + getResources().getString(R.string.packageName) + ":" + appInfo.getPackageName();
    }

    @Override
    public void showAppOperation(AppInfo appInfo) {
